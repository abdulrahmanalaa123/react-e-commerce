...
// but a neat trick is using the initialCheck which was the old way and editing the key of each component to be `SubCategories___${value}___${queryObj.length != 0}`
...
function FilteringComponent() {
  const { queryObj, clearQueries } = useSearchQueries();
  ...
  return (
    ...
    >
      ...
      {Object.keys(queryObj).filter((key) => key !== "pageNo").length != 0 && (
        <button
